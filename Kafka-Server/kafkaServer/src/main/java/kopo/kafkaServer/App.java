...
public class App {
    ...
    public static void main( String[] args ) {
    	...
    	// 전달할 데이터 구조는 Map 구조인 키의 값 형태의 데이터 구조로 메시지를 보냄
    	// 전달할 데이터 구조는 Map 구조인 키의 데이터타입 : 문자열
    	properties.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class.getName());
    	
    	// 전달할 데이터 구조는 Map 구조인 값의 데이터타입 : 문자열
    	...
    	try {
    		do {
    			...
    			for (ConsumerRecord<String, String> record : records) {
    				...
    				// 받은 메시지를 No-SQL DB에 저장하는 등 로직 사용함
    				// 본 실습은 데이터 저장만 함
    				log.info("My Kafka Server Message : " + message);
    			}
    			
    		} while (message.equals(FIN_MESSAGE)); // exit 메세지가 전달될 때까지 계속 동작시킴
    	} catch (Exception e) {
